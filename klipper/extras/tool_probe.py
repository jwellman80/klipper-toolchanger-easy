...
class ToolProbe:
    def __init__(self, config):
        ...
        self.mcu_probe = probe.ProbeEndstopWrapper(config, self.probe_offsets,
                                                   self.param_helper)
...
class ProbeSessionHelper:
    ...
    def start_probe_session(self, gcmd):
        ...
        self.hw_session = self.mcu_probe.start_probe_session(gcmd)
    ...
    def end_probe_session(self):
        ...
        self.results = []
        self.multi_probe_pending = False
        self.hw_session.end_probe_session()
        self.hw_session = None
    def get_probe_params(self, gcmd=None):
        if gcmd is None:
    ...
    def _probe(self, gcmd):
        ...
        try:
            self.hw_session.run_probe(gcmd)
            epos = self.hw_session.pull_probed_results()[0]
